        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl mt-6">
            <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/80">
              ...
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-lg transition-colors text-sm font-medium">
                <Plus size={16} /> Nueva Ruta Favorita
              </button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-950/50 text-slate-400 border-b border-slate-800">
                  <tr>
                    <th className="px-5 py-4 font-medium">Nombre de Ruta</th>
                    <th className="px-5 py-4 font-medium">Origen / Pick-up</th>
                    <th className="px-5 py-4 font-medium">Destino / Drop-off</th>
                    <th className="px-5 py-4 font-medium text-right">Acciones</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/50">
                  <tr className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-5 py-4 font-medium text-slate-200">Bodega Principal a Sucursal</td>
                    <td className="px-5 py-4 text-slate-400 text-xs">Parque Industrial Costa del Este, Ofibodega 4</td>
                    <td className="px-5 py-4 text-slate-400 text-xs">Multiplaza Mall, Local 2B</td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 rounded transition-colors" title="Crear Envío con esta Ruta">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>
                        </button>
                        <button className="p-1.5 bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 rounded transition-colors" title="Eliminar Ruta">
                          <UserX size={16} />
                        </button>
                      </div>
                    </td>
            ...
            <div className="p-5 bg-slate-900/50 border-t border-slate-800 text-xs text-slate-500">
              <p>Las rutas guardadas aparecerán como acceso rápido al momento de crear un <strong>Nuevo Envío</strong>, ahorrándote tiempo al no tener que buscar las direcciones de siempre en el mapa.</p>
            </div>
          </div>
        </div>
